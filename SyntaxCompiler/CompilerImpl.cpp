...
typedef void (*SetCompiler)(Syntax::Compiler::ICompiler *compiler); 
...
{
	to_lower(name);
	hash_map<wstring,NodeHandler*>::iterator iterator = handlers.find(name);
...
{
	wstring errorPath = compilerPath+L"/error_log.txt";
...
{
	wstring outputFileName = dataPath+assetPaths[0];
	wofstream outputFile(outputFileName.c_str());

	topAsset->GenerateFile(outputFile);
}

void CompilerImpl::SetTop(Syntax::Compiler::INodeCompiler *top)
{
	this->top->realTop = top;
}

void Syntax::Compiler::CompilerImpl::CopyFiles(void)
{
	if(outputPaths.size() > 0)
	{
		wpath original = dataPath+outputPaths[0];
		for(unsigned int i = 1;i < outputPaths.size();i++)
		{
			remove(dataPath+outputPaths[i]);
			copy_file(original,dataPath+outputPaths[i]);
		}
	}

	if(assetPaths.size() > 0)
	{
		wpath original = dataPath+assetPaths[0];
		for(unsigned int i = 1;i < assetPaths.size();i++)
		{
			remove(dataPath+assetPaths[i]);
			copy_file(original,dataPath+assetPaths[i]);
		}
	}
}
...
{
	hash_map<wstring,wstring>::const_iterator iterator = settings.find(setting);
